...
class TrackCodeFlow {
    ...
    afterFileValidate(file) {
        ...
        file.parser.references.functionExpressions.forEach((fun) => {
            ...
            const visitStatement = brighterscript_1.createStackedVisitor((stat, stack) => {
                state.stack = stack;
                curr = {
                    stat: stat,
                ...
                if (brighterscript_1.isIfStatement(opened)) {
                    state.ifs = curr;
                }
                else if (!curr.parent || brighterscript_1.isIfStatement(curr.parent)) {
                    state.branch = curr;
                }
                state.parent = curr;
            }, (closed, stack) => {
                ...
                if (brighterscript_1.isIfStatement(closed)) {
            ...
            if (fun.body.statements.length > 0) {
                ...
                fun.body.walk((elem, parent) => {
                    ...
                    if (brighterscript_1.isStatement(elem) && !brighterscript_1.isExpression(parent)) {
...
function findBranch(state) {
    ...
    for (let i = stack.length - 2; i >= 0; i--) {
        if (brighterscript_1.isIfStatement(stack[i])) {
            return {
...
function isBranchedStatement(stat) {
    return brighterscript_1.isIfStatement(stat) || brighterscript_1.isForStatement(stat) || brighterscript_1.isForEachStatement(stat) || brighterscript_1.isWhileStatement(stat);
}
