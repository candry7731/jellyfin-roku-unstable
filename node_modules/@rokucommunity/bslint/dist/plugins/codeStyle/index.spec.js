...
describe('codeStyle', () => {
    ...
    it('enforce return type only', async () => {
        ...
        chai_1.expect(diagnostics[0].range).to.eql(brighterscript_1.util.createRange(4, 0, 4, 8));
    });
    ...
    describe('AA style', () => {
        it('collects wrapping AA members indexes', () => {
            ...
            const indexes = statements.map(s => {
                ...
                return index_1.collectWrappingAAMembersIndexes(value);
            });
            chai_1.expect(indexes).to.deep.equal([
                [],
                [0],
                [0, 1],
                [1, 3],
    ...
    describe('fix', () => {
        ...
        it('replaces `sub` with `function`', async () => {
            ...
            const expected = [];
            chai_1.expect(actual).deep.equal(expected);
            chai_1.expect(lintContext.pendingFixes.size).equals(1);
            await lintContext.applyFixes();
            chai_1.expect(lintContext.pendingFixes.size).equals(0);
            ...
            chai_1.expect(actualSrc).to.equal(expectedSrc);
        });
        ...
        it('removes eol last', async () => {
            const diagnostics = await linter.run(Object.assign(Object.assign({}, project1), { files: ['source/eol-last-temp.brs'], rules: {
                    'eol-last': 'never'
                }, fix: true }));
            const actual = fmtDiagnostics(diagnostics);
            const expected = [];
